...
const Weather = () => {
  ...
  const indianCities = [
    ...
    { city: "Ludhiana", state: "Punjab", lat: 30.901, lon: 75.8573 },
    ...
    { city: "Vasai-Virar", state: "Maharashtra", lat: 19.492, lon: 72.8 },
    ...
    { city: "Navi Mumbai", state: "Maharashtra", lat: 19.033, lon: 73.0297 },
    ...
    { city: "Hubli-Dharwad", state: "Karnataka", lat: 15.3647, lon: 75.124 },
  ...
  const generateForecastData = () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const maxTemp =
        weather.tempRange[0] +
        Math.random() * (weather.tempRange[1] - weather.tempRange[0]);
      const minTemp = maxTemp - (5 + Math.random() * 8);
      const humidity =
        weather.humidity[0] +
        Math.random() * (weather.humidity[1] - weather.humidity[0]);
      const chanceOfRain =
        weather.rain[0] + Math.random() * (weather.rain[1] - weather.rain[0]);
    ...
    const currentLocation =
      indianCities.find(
        (city) =>
          ...
          city.state === selectedLocation.state,
      ) || indianCities[0];
      ...
      const randomizedData = {
        ...
        current: {
          ...
          humidity: Math.max(
            ...
            Math.min(
              95,
              baseData.current.humidity + (Math.random() - 0.5) * 10,
            ),
          ),
          windSpeed: Math.max(
            0,
            baseData.current.windSpeed + (Math.random() - 0.5) * 8,
          ),
        },
      };
  ...
  const getFarmingAdvice = () => {
    ...
    if (current.temperature > 35) {
      advice.push({
        ...
        message:
          "Increase irrigation frequency. Provide shade for sensitive crops. Harvest early morning.",
      ...
      advice.push({
        ...
        message:
          "Protect crops from frost. Use row covers or mulching for sensitive plants.",
        icon: <AlertTriangle className="h-5 w-5" />,
      });
    }
    ...
    if (current.humidity > 85) {
      advice.push({
        ...
        title: "High Humidity",
        message:
          "Monitor for fungal diseases. Ensure good air circulation around plants.",
    ...
    if (rainForecast) {
      advice.push({
        ...
        title: "Rain Expected",
        message:
          "Good time for transplanting. Prepare drainage systems. Reduce irrigation.",
    ...
    if (current.windSpeed > 20) {
      advice.push({
        ...
        title: "Strong Winds",
        message:
          "Secure tall plants and structures. Avoid spraying pesticides or fertilizers.",
        icon: <AlertTriangle className="h-5 w-5" />,
      });
    }
    ...
    if (current.uvIndex > 7) {
      advice.push({
        ...
        title: "High UV Index",
        message:
          "Good for solar drying. Work early morning or late evening to avoid heat stress.",
        icon: <Info className="h-5 w-5" />,
      });
    }
      ...
      <div className="min-h-screen bg-gradient-to-b from-green-50 to-blue-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">
            Getting the latest agricultural weather information
          </p>
        </div>
      </div>
    );
